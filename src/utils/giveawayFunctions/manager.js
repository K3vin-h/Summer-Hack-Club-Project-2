...
module.exports = {
    async createGiveaway(interaction, client) {
        ...
        try {
            ...
            client.logger.error('Failed to fetch guild name:', err);
        };
        try {
            const hostUser = await client.users.fetch(hosted.id);
            hostedTag = `${hostUser.username}`;
        } catch (err) {
            client.logger.error('Failed to fetch hosted user:', err);
        };
        try {
        ...
        try {
            ...
            if (log.channels.giveawayCreate) {
                ...
                if (logChannel) {
                    ...
                    if (giveaway.bonusEntries.length > 0) {
                        logEmbed.addFields({
                            name: 'Bonus Entries',
                            value: giveaway.bonusEntries.map(entry => `<@&${entry.roleId}>: +${entry.entries}`).join('\n'),
                            inline: false,
                        });
                    };
            ...
            return interaction.reply({ content: `Giveaway created in ${channel}!`, flags: MessageFlags.Ephemeral });
        } catch (error) {
            client.logger.error('Giveaway creation error:', error);
    ...
    parseBonusEntries(bonusEntriesStr) {
        try {
            ...
            client.logger.error('Error parsing bonus entries:', error);
            return [];
        };
    },
    ...
    async handleGiveawayJoin(interaction, client) {
        ...
        try {
                        ...
                        embeds: [
                            new EmbedBuilder()
                                ...
                                .addFields(
                                    ...
                                    { name: 'Jump to Giveaway', value: `🔗 [Click here to view Giveaway](https://discord.com/channels/${giveaway.guildId}/${giveaway.channelId}/${giveaway.messageId})`, inline: false }
                                )
                        ],
                        flags: MessageFlags.Ephemeral
                    });
                ...
                return interaction.editReply({
                    embeds: [
                        new EmbedBuilder()
                            ...
                            .setDescription(await requirementsChecker.getRequirementMessage(giveaway, interaction.member, client))
                            .addFields({ name: 'Jump to Giveaway', value: `🔗 [Click here to view Giveaway](https://discord.com/channels/${giveaway.guildId}/${giveaway.channelId}/${giveaway.messageId})`, inline: false })
                    ],
                    flags: MessageFlags.Ephemeral
                });
            };
            ...
            if (log.channels.giveawayEntry) {
                ...
                if (logChannel) {
                    const bonusRoles = giveaway.bonusEntries.filter(entry => interaction.member.roles.cache.has(entry.roleId)).map(entry => `<@&${entry.roleId}> (+${entry.entries})`).join(', ') || 'None';
                    const logEmbed = new EmbedBuilder()
                        .setColor('#2ECC71')
                        ...
                        .addFields(
                            ...
                            { name: 'Bonus Roles', value: bonusRoles, inline: false }
                        )
                        .setTimestamp();
                    await logChannel.send({ embeds: [logEmbed] });
                };
            ...
            client.logger.error('Error handling giveaway join:', err);
            return interaction.editReply({
                embeds: [
                    new EmbedBuilder()
                        .setColor('#E74C3C')
    ...
    async handleGiveawayLeave(interaction, client) {
        ...
        try {
            ...
            client.logger.error('Error leaving giveaway:', error);
            const errorEmbed = new EmbedBuilder()
                .setColor('#E74C3C')
                .setTitle('Error')
    ...
    async updateUserEntries(giveaway, userId, entries) {
        try {
            ...
            client.logger.error('Error updating user entries:', error);
            throw error;
        };
    },
        ...
        try {
            ...
            if (log.channels.giveawayEnd) {
                ...
                if (logChannel) {
                    ...
                    let footerText = `Giveaway ended at${new Date().toLocaleString('en-US', {
                        timeZone: 'UTC',
                        year: 'numeric',
                        month: 'short',
                        day: '2-digit',
                    ...
                    const logEmbed = new EmbedBuilder()
                        .setColor(endedManually ? '#ff9900' : '#E74C3C')
