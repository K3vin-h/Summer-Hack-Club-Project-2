...
async checkRequirements(giveaway, member) {
  if (!giveaway || !member || !member.roles || !member.roles.cache) {
    return false;
  }
  ...
  try {
    ...
    if (giveaway.requiredJoinServerId) {
      try {
        ...
        member.client.logger?.warn?.(`[RequirementsChecker] Failed to check required server (${giveaway.requiredJoinServerId}): ${err.message}`);
        return false;
      }
    }
    ...
    member.client.logger?.error?.(`[RequirementsChecker] Unexpected error in checkRequirements: ${err.message}`);
    return false;
  }
},
